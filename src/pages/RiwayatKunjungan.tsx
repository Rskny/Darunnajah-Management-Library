...
export default function RiwayatKunjungan() {
  ...
  const toggleAll = () => {
    ...
    else setSelected(sorted.map(i => i.id));
  };
  ...
  const deleteSelected = async () => {
    ...
    try {
      // Menggunakan Promise.all agar proses hapus berantai stabil dan tidak memicu error async loop
      await Promise.all(selected.map(id => apiClient.delete(`/visits/${id}`)));
      setSelected([]);
      setSelectMode(false);
      fetchHistory();
  ...
  return (
    ...
    <div data-aos="fade-up" className="p-8 h-screen w-full max-w-full flex flex-col overflow-hidden bg-slate-50">
      ...
      {/* TABEL AREA DENGAN DUKUNGAN GULIR */}
