...
const Riwayat: React.FC = () => {
  ...
  const deleteSelected = async () => {
    ...
    try {
      // Perbaikan eksekusi hapus menggunakan Promise.all demi stabilitas build & network
      await Promise.all(selected.map(id => apiClient.delete(`/transactions/${id}`)));
      setSelected([]);
      setSelectMode(false);
      fetchTransactions();
  ...
  const sorted = [...history]
    .filter((t: any) =>
      ...
      t.bookCode?.toLowerCase().includes(q) ||
  ...
  return (
    // MENAMBAHKAN ANIMASI FADE-UP PADA CONTAINER UTAMA
      ...
      <div className="flex-1 min-h-0 w-full relative">
        <TableBox>
          <div className="absolute inset-0 overflow-auto border border-slate-200 rounded-3xl bg-white shadow-sm">
            <table className="w-full text-sm text-left border-collapse" style={{ minWidth: "1300px" }}>
              <thead className="sticky top-0 z-30 bg-slate-100 text-slate-600 text-xs uppercase">
                <tr>
                  ...
                  <th className="px-6 py-5 bg-slate-100 border-b whitespace-nowrap">ID Anggota</th>
                  <th className="px-6 py-5 bg-slate-100 border-b whitespace-nowrap">Judul Buku</th>
                  <th className="px-6 py-5 bg-slate-100 border-b whitespace-nowrap">Kode Buku</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-xs whitespace-nowrap">
                          {late > 0
                            ? <span className="text-red-500 font-medium italic">Terlambat {late} hari</span>
                            : "-"
                          }
                        </td>
                      </tr>
